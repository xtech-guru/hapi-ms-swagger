  ...
  traverse(options.spec.paths).forEach(function() {
    ...
    if (this.key === 'x-mongo-dql' && this.path.length === 3) {
      ...
      if (route) {
        ...
        if (node.conditionTransform) {
          Hoek.assert(node.conditionTransform.module, `Expected 'conditionTransform.module' to be a non-empty string.`);
          Hoek.assert(node.conditionTransform.functionName, `Expected 'conditionTransform.functionName' to be a non-empty string.`);

          const conditionTransformModule = require(path.join(process.cwd(), node.conditionTransform.module));
          conditionTransform = conditionTransformModule[node.conditionTransform.functionName];
        ...
        route.settings.plugins.mongoDQL = new MongoDQL(conditionTransform, node.sortMappings, node.defaults);
      }
    }
  });
}
